...
if (isset($_POST['add_dean'])) {
    ...
    if (!empty($dean_name) && !empty($faculty_name)) {
        ...
        if ($stmt->execute()) { $msg = "Dean assigned successfully!"; } else { $error = "Error: " . $conn->error; }
        $stmt->close();
        ...
        <div class="row g-4">
            <div class="col-lg-4">
                <div class="card border-0 shadow-sm p-4 rounded-4 bg-white">
                    ...
                    <form method="POST">
                        <div class="mb-3"><label class="form-label small fw-semibold">Dean's Name *</label><input type="text" name="dean_name" class="form-control" required></div>
                        ...
                        <button type="submit" name="add_dean" class="btn btn-info text-white w-100 rounded-pill fw-semibold">Save Dean</button>
                    </form>
                </div>
            </div>
            <div class="col-lg-8">
                <div class="card border-0 shadow-sm p-4 rounded-4 bg-white">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover align-middle">
                            <thead class="table-light"><tr><th>#</th><th>Dean Name</th><th>Faculty</th><th>Contact</th><th class="text-end">Action</th></tr></thead>
                            <tbody>
                                <?php $res = $conn->query("SELECT * FROM deans ORDER BY id DESC"); $i=1;
                                ...
                                <tr>
                                    ...
                                    <td class="text-end"><a href="manage_deans.php?delete_id=<?= $row['id'] ?>" class="btn btn-sm btn-outline-danger" onclick="return confirm('Delete dean?');"><i class="fa-solid fa-trash"></i></a></td>
                                </tr>
                                <?php endwhile; ?>
                            </tbody>
                        </table>
