...
<body>
    ...
    <main class="container py-5">
        <h2 class="fw-bold text-dark mb-4"><i class="fa-solid fa-chalkboard-user text-warning me-2"></i>Manage Teachers
        </h2>
        <?php if (!empty($msg)): ?>
            ...
            <div class="col-lg-4">
                ...
                <ul class="nav nav-pills nav-fill mb-3 bg-white p-1 rounded-4 shadow-sm" id="teacherTab" role="tablist">
                    <li class="nav-item" role="presentation">
                        <button class="nav-item nav-link active rounded-4 fw-semibold small" id="single-tab"
                            data-bs-toggle="pill" data-bs-target="#single-add" type="button" role="tab">Single
                            Add</button>
                    </li>
                    <li class="nav-item" role="presentation">
                        <button class="nav-item nav-link rounded-4 fw-semibold small" id="bulk-tab"
                            data-bs-toggle="pill" data-bs-target="#bulk-add" type="button" role="tab">Bulk CSV
                            Upload</button>
                    </li>
                </ul>
                ...
                <div class="tab-content" id="teacherTabContent">
                    ...
                    <div class="tab-pane fade show active" id="single-add" role="tabpanel">
                        <div class="card border-0 shadow-sm p-4 rounded-4 bg-white">
                            ...
                            <form method="POST">
                                <div class="mb-3"><label class="form-label small fw-semibold">Teacher Name
                                        *</label><input type="text" name="teacher_name" class="form-control" required>
                                </div>
                                <div class="mb-3"><label class="form-label small fw-semibold">Faculty *</label>
                                    <select name="faculty_name" class="form-select" required>
                                        ...
                                        <?php $fr = $conn->query("SELECT DISTINCT faculty_full_name FROM faculty");
                                        while ($f = $fr->fetch_assoc()): ?>
                                            <option value="<?= htmlspecialchars($f['faculty_full_name']) ?>">
                                                <?= htmlspecialchars($f['faculty_full_name']) ?></option>
                                        <?php endwhile; ?>
                                    </select>
                                </div>
                                <div class="mb-3"><label class="form-label small fw-semibold">Contact</label><input
                                        type="number" name="number" class="form-control"></div>
                                <button type="submit" name="add_teacher"
                                    class="btn btn-warning text-dark w-100 rounded-pill fw-semibold">Save
                                    Teacher</button>
                            </form>
                        </div>
                    </div>
                    ...
                    <div class="tab-pane fade" id="bulk-add" role="tabpanel">
                        <div class="card border-0 shadow-sm p-4 rounded-4 bg-white">
                            ...
                            <form method="POST" enctype="multipart/form-data">
                                <div class="mb-3"><label class="form-label small fw-semibold">Faculty *</label>
                                    <select name="bulk_faculty_name" class="form-select" required>
                                        ...
                                        while ($f2 = $fr2->fetch_assoc()):
                                            ?>
                                            <option value="<?= htmlspecialchars($f2['faculty_full_name']) ?>">
                                                <?= htmlspecialchars($f2['faculty_full_name']) ?></option>
                                        <?php endwhile; ?>
                                    </select>
                                </div>
                                <div class="mb-3">
                                ...
                                <button type="submit" name="bulk_upload_teacher"
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
                        <h5 class="fw-bold mb-0">
                            ...
                            <?php if (!empty($filter_faculty)): ?>
                                <span
                                    class="badge bg-primary-subtle text-primary fs-7 rounded-pill ms-2"><?= htmlspecialchars($filter_faculty) ?></span>
                            <?php endif; ?>
                        </h5>
                    </div>
                        ...
                        <div class="col-md-7">
                            ...
                            <select name="faculty_filter" class="form-select form-select-sm rounded-3">
                                ...
                                while ($f_row = $fac_res->fetch_assoc()):
                                    ...
                                    ?>
                                    <option value="<?= htmlspecialchars($f_row['faculty_full_name']) ?>" <?= $sel ?>>
                                        <?= htmlspecialchars($f_row['faculty_full_name']) ?></option>
                                <?php endwhile; ?>
                            </select>
                        </div>
                        <div class="col-md-5 d-flex gap-2 align-self-end">
                            <button type="submit" name="apply_filter"
                                class="btn btn-sm btn-primary rounded-pill px-3 fw-semibold w-100">
                                <i class="fa-solid fa-filter me-1"></i> Filter
                            </button>
                            <?php if (!empty($filter_faculty)): ?>
                                <button type="submit" name="reset_filter"
                                    class="btn btn-sm btn-outline-secondary rounded-pill px-3 fw-semibold w-100">
                                ...
                                if ($res && $res->num_rows > 0):
                                    ...
                                    while ($row = $res->fetch_assoc()):
                                        ...
                                        <tr>
                                            ...
                                            <td><span
                                                    class="badge bg-primary-subtle text-primary"><?= htmlspecialchars($row['faculty_name'] ?? ($row['faculty'] ?? 'N/A')) ?></span>
                                            </td>
                                            <td><?= htmlspecialchars($row['number'] ?? 'N/A') ?></td>
                                            <td class="text-end"><a href="manage_teachers.php?delete_id=<?= $row['id'] ?>"
                                                    class="btn btn-sm btn-outline-danger"
                                                    onclick="return confirm('Delete teacher?');"><i
