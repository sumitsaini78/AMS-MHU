...
if (isset($_POST['add_student'])) {
    ...
    if (!empty($student_name)) {
        ...
        $error = "Student Name is a required field.";
    }
}
...
if (isset($_POST['update_student'])) {
    ...
    if ($can_update) {
        ...
        if ($stmt->execute()) {
            $_SESSION['flash_msg'] = "Student record updated successfully!";
        } else {
            $_SESSION['flash_error'] = "Error updating record: " . $conn->error;
        }
        $stmt->close();
    } else {
...
<body>
    ...
    <main class="container py-5">
        <h2 class="fw-bold text-dark mb-4"><i class="fa-solid fa-user-graduate text-success me-2"></i>Manage Students
        </h2>
        <?php if (!empty($msg)): ?>
            <div class="alert alert-success rounded-4"><?= htmlspecialchars($msg) ?></div><?php endif; ?>
        <?php if (!empty($error)): ?>
            <div class="alert alert-danger rounded-4"><?= htmlspecialchars($error) ?></div><?php endif; ?>
        ...
        <?php if ($edit_data): // --- EDIT FORM --- ?>
            <div class="card border-0 shadow-sm p-4 rounded-4 bg-white mb-4">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <a href="manage_students.php" class="btn btn-sm btn-outline-secondary rounded-pill"><i
                            class="fa-solid fa-xmark me-1"></i> Cancel Edit</a>
                </div>
                <form method="POST" action="manage_students.php">
                    <input type="hidden" name="student_id" value="<?= $edit_data['id'] ?>">
                    <div class="mb-3"><label class="form-label small fw-semibold">Student Name *</label><input type="text"
                            name="student_name" class="form-control" value="<?= htmlspecialchars($edit_data['name']) ?>"
                            required></div>
                    <div class="mb-3"><label class="form-label small fw-semibold">Father's Name</label><input type="text"
                            name="father_name" class="form-control"
                            value="<?= htmlspecialchars($edit_data['father_name'] ?? '') ?>"></div>
                    <div class="row g-2 mb-3">
                        <div class="col-md-6"><label class="form-label small fw-semibold">Enrollment No.</label><input
                                type="text" name="enrollment_number" class="form-control"
                                value="<?= htmlspecialchars($edit_data['enrollment_number'] ?? '') ?>"></div>
                        <div class="col-md-6"><label class="form-label small fw-semibold">Roll No.</label><input type="text"
                                name="roll_number" class="form-control"
                                value="<?= htmlspecialchars($edit_data['roll_number'] ?? '') ?>"></div>
                    </div>
                    <div class="row g-2 mb-3">
                        <div class="col-md-6">
                            <label class="form-label small fw-semibold">Course Name</label>
                            <select name="course_name" class="form-select">
                                <?php mysqli_data_seek($courses_res, 0);
                                while ($c = $courses_res->fetch_assoc()): ?>
                                    <option value="<?= htmlspecialchars($c['course_name']) ?>"
                                        <?= ($edit_data['course'] == $c['course_name']) ? 'selected' : '' ?>>
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="semester" class="form-select">
                                <?php for ($i = 1; $i <= 10; $i++): ?>
                                    <option value="<?= $i ?>" <?= ($edit_data['sem'] == $i) ? 'selected' : '' ?>><?= $i ?></option>
                                <?php endfor; ?>
                            </select>
                        </div>
                    </div>
                    <div class="mb-3">
                        ...
                        <select name="faculty_name" class="form-select" required>
                            <?php mysqli_data_seek($faculties_res, 0);
                            while ($f = $faculties_res->fetch_assoc()): ?>
                                <option value="<?= htmlspecialchars($f['faculty_full_name']) ?>"
                                    <?= ($edit_data['faculty'] == $f['faculty_full_name']) ? 'selected' : '' ?>>
                                    <?= htmlspecialchars($f['faculty_full_name']) ?>
                                </option>
                            <?php endwhile; ?>
                        </select>
                    </div>
                    <button type="submit" name="update_student"
                        class="btn btn-primary w-100 rounded-pill fw-semibold">Update Student</button>
                </form>
            </div>
        <?php else: // --- ADD FORMS --- ?>
            <div class="accordion mb-4" id="studentFormsAccordion">
                    ...
                    <div id="collapseOne" class="accordion-collapse collapse" data-bs-parent="#studentFormsAccordion">
                        <div class="accordion-body">
                            ...
                            <form method="POST" class="needs-validation" novalidate>
                                <div class="mb-3"><label class="form-label small fw-semibold">Student Name *</label><input
                                        type="text" name="student_name" class="form-control" required></div>
                                <div class="mb-3"><label class="form-label small fw-semibold">Father's Name</label><input
                                        type="text" name="father_name" class="form-control"></div>
                                <div class="row g-2 mb-3">
                                    <div class="col-md-6"><label class="form-label small fw-semibold">Enrollment
                                            No.</label><input type="text" name="enrollment_number" class="form-control"
                                            placeholder="Optional"></div>
                                    <div class="col-md-6"><label class="form-label small fw-semibold">Roll No.</label><input
                                            type="text" name="roll_number" class="form-control" placeholder="Optional">
                                    </div>
                                </div>
                                <div class="row g-2 mb-3">
                                    <div class="col-md-6"><label class="form-label small fw-semibold">Course
                                            Name</label><select name="course_name" class="form-select">
                                    ...
                                    <div class="col-md-6"><label
                                            class="form-label small fw-semibold">Semester</label><select name="semester"
                                            class="form-select">
                                ...
                                <div class="mb-3"><label class="form-label small fw-semibold">Faculty</label><select
                                        name="faculty_name" class="form-select" required>
                                        <option value="" selected disabled>Select Faculty</option>
                                        <?php mysqli_data_seek($faculties_res, 0); ?>
                                        <?php while ($f = $faculties_res->fetch_assoc()): ?>
                                            <option value="<?= htmlspecialchars($f['faculty_full_name']) ?>">
                                                <?= htmlspecialchars($f['faculty_full_name']) ?>
                                            </option><?php endwhile; ?>
                                    </select></div>
                                <button type="submit" name="add_student"
                                    class="btn btn-success w-100 rounded-pill fw-semibold">Save Student</button>
                            </form>
                        </div>
                    </div>
                </div>
                <div class="accordion-item mt-3 rounded-4 border-0 shadow-sm">
                    ...
                    <div id="collapseTwo" class="accordion-collapse collapse" data-bs-parent="#studentFormsAccordion">
                        <div class="accordion-body">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <h5 class="fw-bold mb-0">Bulk CSV Upload</h5><a href="?download_sample=1"
                                    class="btn btn-sm btn-outline-secondary rounded-pill" style="font-size:0.75rem;"><i
                                        class="fa-solid fa-download me-1"></i> Sample CSV</a>
                            </div>
                            <form method="POST" enctype="multipart/form-data">
                                <div class="mb-3"><label class="form-label small fw-semibold">Select CSV File
                                        *</label><input type="file" name="csv_file" class="form-control" accept=".csv"
                                        required></div>
                                <div class="p-2 bg-light rounded-3 mb-3 small text-muted border"><strong>Format:</strong>
                                    Name, Father Name, Enroll, Roll, Faculty, Course, Section, Year, Sem, Admission Date,
                                    Session</div>
                                <button type="submit" name="import_csv"
                                    class="btn btn-success w-100 rounded-pill fw-semibold"><i
                                        class="fa-solid fa-upload me-1"></i> Upload Data</button>
                            </form>
                        </div>
                    </div>
                </div>
                        ...
                        <form method="GET" class="d-flex" style="max-width: 300px;">
                            <div class="input-group">
                                ...
                                <?php if (!empty($search_term)): ?>
                                    <a href="manage_students.php" class="btn btn-outline-secondary" title="Reset Search"><i
                                        ...
                                        <tr>
                                            <td>
                                                ...
                                                <div class="small text-muted">
                                                    <?= htmlspecialchars($row['father_name'] ?? 'N/A') ?>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="small">Enroll:
                                                    <code><?= htmlspecialchars($row['enrollment_number'] ?: 'N/A') ?></code>
                                                </div>
                                                <div class="small">Roll:
                                                    <code><?= htmlspecialchars($row['roll_number'] ?: 'N/A') ?></code>
                                                </div>
                                            </td>
                                            <td>
                                                <div class="fw-semibold"><?= htmlspecialchars($row['course'] ?? 'N/A') ?></div>
                                                <div class="small text-muted">Sem: <?= htmlspecialchars($row['sem'] ?? 'N/A') ?>
                                                </div>
                                            </td>
                                            <td><span
                                                    class="badge bg-primary-subtle text-primary fw-semibold"><?= htmlspecialchars($row['faculty'] ?? 'N/A') ?></span>
                                            </td>
                                            <td class="text-end">
                                                <a href="manage_students.php?edit_id=<?= $row['id'] ?>"
                                                    class="btn btn-sm btn-outline-primary" title="Edit Student"><i
                                                        class="fa-solid fa-pencil"></i></a>
                                                <a href="manage_students.php?delete_id=<?= $row['id'] ?>"
                                                    class="btn btn-sm btn-outline-danger"
                                                    onclick="return confirm('Delete student?');" title="Delete"><i
