...
if (isset($_POST['add_subject'])) {
    ...
    if (!empty($subject_name) && !empty($semester) && !empty($subject_code) && !empty($course_name)) {
        // Upsert statement matching the composite unique constraint: (course_name, semester, subject_code)
        $stmt = $conn->prepare("INSERT INTO subjects (course_name, Year, semester, subject_name, subject_code, faculty_name) VALUES (?, ?, ?, ?, ?, ?) ON DUPLICATE KEY UPDATE Year = VALUES(Year), subject_name = VALUES(subject_name), faculty_name = VALUES(faculty_name)");
        ...
        $error = "Course Name, Subject Name, Subject Code, and Semester are required fields."; 
    }
}
    ...
    if (isset($_FILES['csv_file']) && $_FILES['csv_file']['error'] == 0) {
        ...
        while (($data = fgetcsv($handle, 1000, ",")) !== FALSE) {
            if (count($data) >= 5) {
                ...
                if (empty($course) || empty($sub_name) || empty($sem) || empty($sub_code)) {
                    continue;
                }

                // Unique key combining Course Name + Semester + Subject Code
                $csv_unique_key = mb_strtolower($course) . '_' . $sem . '_' . mb_strtolower($sub_code);
...
<body>
    ...
    <main class="container py-5">
        ...
        <?php if (!empty($msg)): ?><div class="alert alert-success rounded-4 shadow-sm"><?= htmlspecialchars($msg) ?></div><?php endif; ?>
        <?php if (!empty($error)): ?><div class="alert alert-danger rounded-4 shadow-sm"><?= htmlspecialchars($error) ?></div><?php endif; ?>
                    ...
                    <div class="tab-pane fade show active" id="single-add" role="tabpanel">
                        <div class="card border-0 shadow-sm p-4 rounded-4 bg-white">
                            ...
                            <form method="POST">
                                ...
                                <div class="mb-3"><label class="form-label small fw-semibold">Subject Code *</label><input type="text" name="subject_code" class="form-control" placeholder="e.g., BBA25-101T" required></div>
                                <div class="mb-3">
                                ...
                                <div class="mb-3"><label class="form-label small fw-semibold">Year</label><input type="number" name="year" class="form-control" value="<?= date('Y') ?>" required></div>
                                ...
                                <div class="mb-3"><label class="form-label small fw-semibold">Faculty Name</label><input type="text" name="faculty_name" class="form-control" placeholder="e.g., FOCBS"></div>
                ...
                <div class="card border-0 shadow-sm p-4 rounded-4 bg-white">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover align-middle">
                            <thead class="table-light"><tr><th>#</th><th>Subject Name</th><th>Code</th><th>Course</th><th>Year/Sem</th><th class="text-end">Action</th></tr></thead>
                            <tbody>
                                <?php 
                                ...
                                if ($res && $res->num_rows > 0) {
                                    ...
                                    <tr>
                                        ...
                                        <td><span class="badge bg-light text-dark border"><?= htmlspecialchars($row['subject_code'] ?? 'N/A') ?></span></td>
                                        <td><?= htmlspecialchars($row['course_name'] ?? 'N/A') ?></td>
                                        <td><?= htmlspecialchars($row['Year'] ?? '') ?> / Sem <?= htmlspecialchars($row['semester'] ?? 'N/A') ?></td>
